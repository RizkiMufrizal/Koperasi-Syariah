 ...
 * @Last Modified time: 2016-08-19 18:56:13
 */
...
class AnggotaController extends CI_Controller
{
    ...
    {
        $userCheck = $this->User->ambilSatuUser($this->input->post('username'));

        if ($userCheck != null) {
            $this->session->set_flashdata('pesan', 'maaf, username telah digunakan');
            return redirect('admin/AnggotaController/tambahAnggota');
        } else {
            $role = $this->session->userdata('role');
            if ($role == 'ROLE_USER') {
                $this->session->set_flashdata('pesan', 'maaf, anda tidak memiliki hak akses untuk halaman tersebut');
